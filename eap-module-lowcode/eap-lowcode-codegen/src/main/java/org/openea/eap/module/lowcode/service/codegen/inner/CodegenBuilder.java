...
import org.openea.eap.module.lowcode.convert.codegen.CodegenConvert;
...
import org.openea.eap.module.lowcode.enums.codegen.CodegenColumnHtmlTypeEnum;
import org.openea.eap.module.lowcode.enums.codegen.CodegenColumnListConditionEnum;
import org.openea.eap.module.lowcode.enums.codegen.CodegenTemplateTypeEnum;
...
public class CodegenBuilder {
    ...
    private void initTableDefault(CodegenTableDO table) {
        ...
        table.setModuleName(CharSequenceUtil.subBefore(tableName, '_', false).toLowerCase());
        // 第一步，第一个 _ 前缀的后面，作为 module 名字; 第二步，可能存在多个 _ 的情况，转换成驼峰; 第三步，businessName 必须小写；
        table.setBusinessName(CharSequenceUtil.toCamelCase(CharSequenceUtil.subAfter(tableName, '_', false)).toLowerCase());
        // 驼峰 + 首字母大写；第一步，第一个 _ 前缀的后面，作为 class 名字；第二步，驼峰命名
        table.setClassName(CharSequenceUtil.upperFirst(CharSequenceUtil.toCamelCase(CharSequenceUtil.subAfter(tableName, '_', false))));
        // 去除结尾的表，作为类描述
    ...
    private void processColumnExample(CodegenColumnDO column) {
        ...
        if (StrUtil.endWithAnyIgnoreCase(column.getJavaField(), "id", "price", "count")) {
            column.setExample(String.valueOf(RandomUtil.randomInt(1, Short.MAX_VALUE)));
            return;
        }
        // name
        if (StrUtil.endWithIgnoreCase(column.getJavaField(), "name")) {
            column.setExample(RandomUtil.randomEle(new String[]{"张三", "李四", "王五", "赵六", "二麻子"}));
            return;
        }
        // status
        if (StrUtil.endWithAnyIgnoreCase(column.getJavaField(), "status", "type")) {
            column.setExample(RandomUtil.randomEle(new String[]{"1", "2"}));
            return;
        }
        // url
        ...
        if (StrUtil.endWithIgnoreCase(column.getColumnName(), "reason")) {
            column.setExample(RandomUtil.randomEle(new String[]{"不喜欢", "不对", "不好", "不香"}));
            return;
        }
        // description、memo、remark
        if (StrUtil.endWithAnyIgnoreCase(column.getColumnName(), "description", "memo", "remark")) {
            column.setExample(RandomUtil.randomEle(new String[]{"内部说明", "提示"}));
            return;
        }
    }
