...
public class MyLogicDeleteInnerInterceptor extends JsqlParserSupport implements InnerInterceptor {
    ...
    public void beforeQuery(Executor executor, MappedStatement ms, Object parameter, RowBounds rowBounds, ResultHandler resultHandler, BoundSql boundSql) throws SQLException {
        if (InterceptorIgnoreHelper.willIgnoreOthersByKey(ms.getId(), LOGIC_DELETE_KEY)) return;
        try {
            if(boundSql.getSql().toLowerCase().contains(logicDeleteHandler.getLogicDeleteColumn().toLowerCase())){
                //mybatis plus 自带方法会自动添加字段
                //包含逻辑删除字段不处理
                return;
            }
    ...
    public void beforePrepare(StatementHandler sh, Connection connection, Integer transactionTimeout) {
        ...
        if (sct == SqlCommandType.INSERT || sct == SqlCommandType.UPDATE || sct == SqlCommandType.DELETE) {
            if (InterceptorIgnoreHelper.willIgnoreOthersByKey(ms.getId(), LOGIC_DELETE_KEY)) {
                return;
            }
    ...
    protected void processInsert(Insert insert, int index, String sql, Object obj) {
        ...
        if (select != null) {
            ...
            if (itemsList instanceof MultiExpressionList) {
                ((MultiExpressionList) itemsList).getExpressionLists().forEach(el -> el.getExpressions().add(logicDeleteHandler.getNotDeletedValue()));
            } else {
                ((ExpressionList) itemsList).getExpressions().add(logicDeleteHandler.getNotDeletedValue());
            }
        } else {
    ...
    protected Expression andExpression(Table table, Expression where) {
        //获得where条件表达式
        Expression logicExpression = getLogicExpression(this.getAliasColumn(table), logicDeleteHandler.getNotDeletedValue());
        if (null != where) {
            if (where instanceof OrExpression) {
                return new AndExpression(logicExpression, new Parenthesis(where));
            } else {
                return new AndExpression(logicExpression, where);
            }
        }
     ...
     * <p>
     * 进入这里表示需要 insert 的表启用了逻辑删除,则 select 的表都启动了
     *
    ...
    protected void appendSelectItem(List<SelectItem> selectItems) {
        ...
        selectItems.add(new SelectExpressionItem(new Column(logicDeleteHandler.getLogicDeleteColumn())));
    }
    ...
    protected Expression builderExpression(Expression currentExpression, List<Table> tables) {
        ...
        List<Expression> equalsTos = tempTables.stream()
                .map(item -> getLogicExpression(getAliasColumn(item), notDeletedValue))
                .collect(Collectors.toList());
